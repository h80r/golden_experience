...
### [ ] F8-T5: Correção - Golden Experience em Permissões de Notificação
...
- [ ] `AndroidManifest.xml` corrigido com declarações necessárias
- [ ] Permissões adicionadas corretamente
- [ ] App aparece em Configurações > Acesso às notificações
- [ ] Usuário consegue conceder permissão manualmente
- [ ] Service inicia e funciona após permissão concedida
- [ ] Testes manuais em dispositivo físico/emulador
- [ ] Merge realizado para `develop`
...
## 🏗️ Fase 9: Estabilidade, Code Health e Migração (41 Issues)
...
### [ ] F9-T1: Atualização Crítica de Dependências (30+ Issues)
...
### [ ] F9-T2: Migração de APIs Deprecated e Contextos Assíncronos (11 Issues)
...
### [ ] F9-T3: Implementação de Logging e Limpeza de Produção (7 Issues)
...
### [ ] F9-T4: Remoção de Código Morto e Alertas de Compilação (10 Issues)
...
Este plano mapeia todas as **37 tarefas** necessárias para completar o MVP do Previsor Financeiro. Ao seguir este roadmap, você terá um aplicativo funcional, testado e preparado para uso pessoal, com uma arquitetura sólida que permitirá expansões futuras.
...
A **Fase 9** garante a estabilidade e saúde do código: atualização de dependências, migração de APIs deprecated, implementação de logging adequado, e remoção de código morto.
