...
import '../database/invoices_table.dart';
...
class LocalDatabase extends _$LocalDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (Migrator m, int from, int to) async {
          ...
          // Migration from v11 to v12: Add Invoices and InvoiceItems tables
          if (from <= 11) {
            await m.createTable($InvoicesTable(this));
            // Note: InvoiceItems table was removed in v13
          }

          // Migration from v12 to v13: Simplify Invoices table schema
          // Remove InvoiceItems table and simplify Invoices to only store period + isPaid
          if (from <= 12) {
            // Drop InvoiceItems table if it exists (cascade delete will handle foreign keys)
            await customStatement('DROP TABLE IF EXISTS invoice_items');

            // Recreate Invoices table with simplified schema
            await customStatement('DROP TABLE IF EXISTS invoices');

            // Create new Invoices table with only startDate, endDate, isPaid
            await customStatement('''
